...
<?php
    ...
    $limt=count($vec);
		...
		$incr1=(($navigator["browser"]==='CHROME')?20:(($navigator["browser"]==='IE')?20:20));//
/**/
		$i=0;
		$cc=1;
		while ($i<=0){
			...
			while ($y<=0){
				...
				$trHtml=$trHtml.'<tr>
									...
									<td class="tdText3">'.$vec[0]->tipo.'</td>
									<td style="" class="tdText2">('.$vec[0]->id_art.') '.$vec[0]->descr.'</td>
									<td class="tdText4">'.$vec[0]->marca.'</td>
									<td class="tdText01">'.$vec[0]->cant01.'</td>
									<td class="tdText3">pz</td>
									<td class="tdText3" style="width:15px;"></td>
								</tr>';
								/*    width: 700px;*/

...
/***********************************VALES*********************************************************/
    ...
    tb_genVale=$('#tb_genVale').DataTable({
        ...
        columns: [
          ...
          { data: null,render: function ( data, type, row, meta ) {
                return `<table><tr>
                        <td>$</td>
                        <td><input type="text" class="form-control form-control-sm" value="`+data.precio+`" onclick="fn_insertPrecio(this,event)" onkeyup="fn_insertPrecio(this,event)"  clave="`+data.id+`" style="font-size: 12px; text-align: right;"></td>
                      </tr></table>`;
              } 
          },
          { data: null,render: function ( data, type, row, meta ) {
                return '<input type="text" class="form-control form-control-sm" value="'+data.posicion+'" onclick="fn_insertPosicion(this,event)" onkeyup="fn_insertPosicion(this,event)"  clave="'+data.id+'" style="font-size: 12px;" >';
              } 
          },
          { data: null,render: function ( data, type, row, meta ) {
  ...
  function fn_insertPosicion(id,e){
    let enterKey = 13;
    if (e.which == enterKey){
      $("#inpVale_art").val("");
      $("#inpVale_art").focus();
...
/***********************************CATEGORIAS***************************************************/
                        ...
                        <div class="container-fluid">
                          <div class="row">
                            ...
                            <div class="col-sm-3 col-md-3 col-lg-3 col-xl-3">
                              ...
                              <button class="btn btn-info mt-2" onclick="fn_excelUrl()" 
  ...
  function fn_llenadoIni(){
    listArt_1.sort(function (a, b) {
      return a.nom.localeCompare(b.nom);
    });
    let list=listArt_1;
    ...
    for(let f=0;f<list.length;f++){
        let row=list[f];
        html3=html3+`<div id="colCard_`+row.id+`" class="col-sm-6 col-md-6 col-lg-3 col-xl-3">
                        <div class="tabCard">
                          <div class="h_tabCard">
                            <table style="width: 100%; font-size: 0.8em;">
                                  <tbody>
                                    ...
                                    <tr>
                                      ...
                                      <td   idE="`+row.id+`" numImg="`+f+`" close="0" onclick="fn_masInfoArtDB(this,0)" class="tdIcon" title="Expandir">
                          ...
                          <div id="" class="b_tabCard `+row.id+`">
                            <table style="width: 100%; font-size: 0.8em;">
                              <tbody>
                                <tr>
                                 <th>Modelo:</th>
                                 <td id="tjt_modelo_`+row.id+`"></td>
                                 <th>Unidad:</th>
                                 <td id="tjt_uni_`+row.id+`"></td>
                                </tr>
                                <tr>
                                  <th colspan="4">Observación:</th>
                                </tr>
                                <tr>
                                  <td colspan="4" id="tjt_obs_`+row.id+`"></td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                  <th>Altura:</th>
                                  <td id="tjt_alt_`+row.id+`"></td>
                                  <th>Ancho:</th>
                                  <td id="tjt_anc_`+row.id+`"></td>
                                </tr>
                                <tr>
                                  <th>Profund.:</th>
                                  <td id="tjt_prof_`+row.id+`"></td>
                                  <th>Peso:</th>
                                  <td id="tjt_peso_`+row.id+`"></td>
                                  </tr>
                                </tbody>
                            </table>
                          </div>  
                              ...
                              <thead>
                                ...
                                `+((parametors.usu=='M')
                                      ?`<tr>
                                          ...
                                          <td colspan="2">`+fn_notaPrecio(row.costo.toFixed(2))+`</td>
                                          
                                        </tr>`
                                      :`<tr><th colspan="2"></th><th colspan="2"></th></tr>`
                                  )+`
                                <tr>
                                  <td class="tdIcon" title="" onclick="fn_masInfoArtDB(this,1)" idE="`+row.id+`">
  ...
  function fn_filtTipo(id){
    ...
    for(let d=0;d<vec.length;d++){
      let art=vec[d];
      if(menuOp==0){
        $("#colCard_"+art.id).removeClass('notCar'); 
      }else{
        if(menuOp==art.id_tipo){
            $("#colCard_"+art.id).removeClass('notCar'); 
        }else{
            $("#colCard_"+art.id).addClass('notCar');
        }
      }  
    }
  }
...
/************************************ARTICULOS*************************************************/
  ...
  function fn_clean_mdlNewElemt(){
    ...
    $("#frm_newEle_costo").val("PR");
    $("#frm_newEle_rent").val(0);
    $("#frm_newEle_tipRent").val(1);

  }
  function fn_editElemt(id,idCat){
    $('#mdl_newElemt').modal('toggle');
  ...
  function fn_buscarElemt(event){
    let vec=listArt_1;
    var x = event.which || event.keyCode;
    if(x==33 || x==34 || x==35 || x==36 || x==37 || x==38 || x==39 || x==40){
    }else{  
      ...
      if(arts.length>0){
        for(let d=0;d<vec.length;d++){
          ...
          let ok=false;
          if(menuOp==0){
            fn_filtrarArtMarca(art.id,art.nom,art.marcas,arts,marcs);
          }else{
            if(menuOp==art.id_tipo){
              fn_filtrarArtMarca(art.id,art.nom,art.marcas,arts,marcs);
            }else{
        ...
        if(marcs.length>0){
          for(let d=0;d<vec.length;d++){
            ...
            let ok=false;
            if(menuOp==0){
              fn_filAM(art.marcas,marcs,art.id);
            }else{
              if(menuOp==art.id_tipo){
                fn_filAM(art.marcas,marcs,art.id);
              }else{
  ...
  function fn_filtrarArtMarca(id,art,marca,vecA,vecM){
    let ok=false;
    for(let a=0;a<vecA.length;a++){
      ok=art.toLowerCase().includes(vecA[a].toLowerCase());
      if(!ok){ break; }
    }
    if(ok){
      if(vecM.length>0){
        fn_filAM(marca,vecM,id);
      }else{
        $("#colCard_"+id).removeClass('notCar'); 
      }  
    }else{
      $("#colCard_"+id).addClass('notCar');
    }  
  }

  function fn_filAM(nom,vec,id){
    let ok=false;
    for(let a=0;a<vec.length;a++){
        ok=nom.toLowerCase().includes(vec[a].toLowerCase());
        if(!ok){ break; }
    }
    if(ok){
        $("#colCard_"+id).removeClass('notCar'); 
  ...
  function fn_mdlPositionArt(id,vec,vec2){
    ...
    if(vec2.length>0){
      ...
      for(let f=0;f<vec2.length;f++){
          ...
          for(let d=0;d<vec.length;d++){
            let alm=vec[d];
            console.log(row.id_aloj,alm.id);
            if(row.id_aloj==alm.id){
              html1=html1+`<tr id="`+alm.id+`"><td>(`+alm.id+`) `+alm.nom+`</td>
  ...
  function fn_expandirInfoElemt(ind){
      ...
      if(close){
        ...
        $(ind).attr("onclick","fn_expandirInfoElemt(this)");
        ...
        $(ind).attr("onclick","fn_masInfoArtDB(this,0)");
        let numImg=$(ind).attr("numImg");
...
/************************************MARCAS*************************************************/
    ...
    if(x==33 || x==34 || x==35 || x==36 || x==37 || x==38 || x==39 || x==40){
      ...
      if(marcs.length>0){
        for(let d=0;d<vec.length;d++){
          ...
          let ok=false;
          if(menuOp==0){
            fn_filtrarMarcaArt(art.id,art.nom,art.marcas,arts,marcs);
          }else{
            if(menuOp==art.id_tipo){
              fn_filtrarMarcaArt(art.id,art.nom,art.marcas,arts,marcs);
            }else{
  ...
  function fn_filtrarMarcaArt(id,art,marca,vecA,vecM){
    let ok=false;
    for(let a=0;a<vecM.length;a++){
      ok=marca.toLowerCase().includes(vecM[a].toLowerCase());
      if(!ok){ break; }
    }
    if(ok){
      if(vecA.length>0){
        fn_filAM(art,vecA,id);
      }else{
        $("#colCard_"+id).removeClass('notCar');   
      }
    }else{
...
function fn_evenModals(){
  ...
  $('#mdl_ExcelUrl').on('hidden.bs.modal', function (e) {
    $("#id_loadarch").addClass("notCar");
    $("#id_msgArch").addClass("notCar");
    vecPrec=[];
  });
...
function fn_excelUrl(){
 $("#mdl_ExcelUrl").modal("show");
 $('#inpUrlExcel').change(function() {
    var filename = $('#inpUrlExcel').val();
...
function fn_subirArch(form){
    //inicio
    let peticion= new XMLHttpRequest();
    //progreso
    peticion.upload.addEventListener("progress",(vent)=>{
      console.log("1");
      $("#id_loadarch").removeClass("notCar");
    });
    //finalizo
    peticion.addEventListener("load",()=>{
      console.log("2");
      $("#id_loadarch").addClass("notCar");
      $("#id_msgArch").removeClass("notCar");
      fn_excelJson();
    });
    //enviar datos
    peticion.open("post","../Code/Controller/sube.php");
    peticion.send(new FormData(form));
}


var vecPrec=[];
function fn_excelJson(){
  var url = "test.xlsx";
  console.log(url);
  ...
  oReq.onload = function(e) {
    ...
    vecPrec=XLSX.utils.sheet_to_json(worksheet,{raw:true});
    console.log(vecPrec);
  }
